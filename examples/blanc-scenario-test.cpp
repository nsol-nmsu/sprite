...
void onFind(std::string node,  uint32_t txid, bool payer);
void onTxFail(uint32_t txid, uint32_t node1, uint32_t node2);
...
int main (int argc, char *argv[])
{
  ...
  int run = 0, Method2 = 1, Lighting = 0;	
  CommandLine cmd;
  ...
  tracefile.open(prefix+"-Base-trace"+ std::to_string(run) +".csv", std::ios::out);
  tracefile << "event,txID,time" << std::endl;

  tracefile1.open(prefix+"-Base-path"+ std::to_string(run) +".csv", std::ios::out);
  ...
  if (configFile.is_open ()) {
     while (std::getline(configFile, strLine)) {
        ...
        if(gettingNodeCount == true) {
           ...
           auto apps = nodes.Get( std::stoi( netParams[0] ) )->GetApplication( 0 )->GetObject<ns3::Blanc>();
           ...
           if(Lighting){
              ...
              linkFrom = std::stod(netParams[3])/ 1000000;
                ...
                if( RHs[std::stoi(netParams[0])] == 1  ||  RHs[std::stoi(netParams[1])] == 1){
                        linkTo = rh_links[  rand()%rh_links.size()  ];
                        linkFrom = rh_links[  rand()%rh_links.size()  ];
                }
           }
  ...
  for ( int i=0; i<( int )nodes.size()-1; i++ ) {
	...
	auto apps = nodes.Get( i )->GetApplication( 0 )->GetObject<ns3::Blanc>();
        sync.addNode( i,apps );
	...
	strcallback = "/NodeList/" + std::to_string(i) + "/ApplicationList/" + "*/OnPay";
        ...
        strcallback = "/NodeList/" + std::to_string(i) + "/ApplicationList/" + "*/OnFind";
        Config::ConnectWithoutContext( strcallback, MakeCallback( &onFind ) );        
...
void onFind(std::string node,  uint32_t txid, bool payer){
   if (payer) {
        tracefile << "Find,"<< txid << "," << ns3::Simulator::Now().GetSeconds() << std::endl;
        tracefile1 << "Find,"<< txid << "," << ns3::Simulator::Now().GetSeconds() << ","<< node << ","<< std::endl;
   }
   else
        tracefile1 << "Find,"<< txid << "," << ns3::Simulator::Now().GetSeconds() << ",,"<< node << std::endl;
};
...
void onTxFail(uint32_t txid, uint32_t node1, uint32_t node2){
   ...
   tracefile1 << "Fail," << txid<<","<<ns3::Simulator::Now().GetSeconds() << "," << node1 << "," << node2 << std::endl;
   sync.onTxFail(txid);	
};
