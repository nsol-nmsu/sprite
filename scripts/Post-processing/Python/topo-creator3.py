...
def main(seed=None):
 ...
 for router_helper in list_routerhelper:
    ...
    while level < neigborhood_hops:
        ...
        for n in node_level_list[level]:
            ...
            if level+1 == neigborhood_hops:
                links = random.choice([0,0,0,1])
            while links > 0:
                links-=1
                chosenLevel = level
                cont = True
        ...
        if level == neigborhood_hops:
            ...
            list_neighborhood_edge[router_helper] = copy.copy(node_level_list[level-1])
 ...
 for n in list_client_edge_router_copy:
        ...
        if distance > min_c2r_hops and G.degree(n)==1:            
 ...
 for n in list_client_edge_router:
    ...
    node_to_RH_map[current_highest_node] = node_to_RH_map[n]
    G.add_node(current_highest_node,
        role="client",
        name=current_highest_node)
 ...
 connect_list = []
 for rh in list_neighborhood_edge:
    for e in list_neighborhood_edge[rh]:
        connect_list.append(e)
 ...
 for n in connect_list:
        ...
        print(n,x)
        attach = random.choice([False,True,True,True])
        if attach:
            G.add_edge(n, x,
                Delay="10ms",   
                DataRate="5Mbps",
 ...
 for each in list_of_clients:
     proxies_file.write(str(each)+" "+str(node_to_RH_map[each])+"\n");
